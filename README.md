...
내 얼굴을 한 번 스캔해두면, 저장된 **내 3D 얼굴**을 돌려보면서 눈 · 코 · 턱/윤곽 · 입술 · 피부 · 리프팅 변화를
...
4. 스튜디오: 부위 탭 → 프리셋 또는 슬라이더, 되돌리기/다시하기, **원본 보기**를 누르고 있으면 원본
   (눈 탭: 내 눈 분석, 쌍꺼풀 · 트임 · 눈매교정 · 눈가 볼륨)
5. **적용하기** → 비교하기: 나란히 / 슬라이더 / 번갈아 보기, 각도 선택, 오른쪽 위 버튼으로 비교 이미지 저장
6. **이 룩 저장하기** → 내 룩 목록 → **비교**로 다시 열기 (Before 라벨을 누르면 다른 룩과 비교)

## 눈 시뮬레이션

- **내 눈 분석**: 정면 사진의 홍채 가로 지름(성인 약 11.7mm)을 자로 삼아 눈 가로 길이, 눈 사이 거리,
  눈꼬리 각도, 눈 뜨는 정도를 mm로 추정해요(`services/api/app/reconstruction/eyes.py`).
- **쌍꺼풀**: 속눈썹선에서 5~9mm 위, 인라인 · 인아웃 · 아웃라인 라인으로 접힘을 그려요(셰이더).
- **눈매교정 · 앞트임 · 뒷트임 · 밑트임**: 눈꺼풀과 눈꼬리를 실제 mm만큼 옮겨요. 눈 구멍 안쪽은 서버가 만든
  "깨끗한 안구" 텍스처(가려진 홍채 · 흰자 · 눈물언덕 복원, `eyeball.py`)를 안구에 고정된 좌표로 읽어서,
  눈꺼풀이 열리면 늘어나는 대신 가려져 있던 눈이 드러나요.
- 눈 분석과 안구 텍스처는 이 기능 이후에 스캔한 얼굴에만 있어요. 이전 스캔은 다시 스캔하면 생겨요.
