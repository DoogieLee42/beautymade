...
def render_view(model: dict, albedo_rgb: np.ndarray, yaw_deg: float, size: int = 960) -> np.ndarray:
    """Software-renders the textured face turned by `yaw_deg` (positive = to the subject's left)."""
    ...
    tex = albedo_rgb.shape[0]
    uv_px = np.stack([uvs[:, 0] * tex, (1 - uvs[:, 1]) * tex], 1).astype(np.float32)

    a = math.radians(yaw_deg)
    rot = np.array([[math.cos(a), 0, math.sin(a)], [0, 1, 0], [-math.sin(a), 0, math.cos(a)]])
    pts = positions @ rot.T
    scale = size / 25.0
    xy = np.stack([pts[:, 0] * scale + size / 2, -pts[:, 1] * scale + size / 2], 1).astype(np.float32)
    img = np.full((size, size, 3), (118, 112, 108), np.uint8)
